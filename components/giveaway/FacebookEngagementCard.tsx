...
import { Facebook, ThumbsUp, MessageCircle, ExternalLink, RefreshCw, Users } from "lucide-react";
import { fetchFacebookEngagement, summarizeFacebookEngagement, facebookCriteriaLabel } from "@/lib/services/facebook";
import { formatNumber } from "@/lib/utils";
import type { FacebookCriteriaType, FacebookEngagement, FacebookSource } from "@/types";

interface FacebookEngagementCardProps {
  source: FacebookSource;
  primaryColor: string;
  secondaryColor: string;
}

const CRITERIA_ICONS: Record<FacebookCriteriaType, typeof ThumbsUp> = {
  comment: MessageCircle,
  like: ThumbsUp,
};

/**
 * Публичная витрина розыгрыша "по посту Facebook": показывает пост и живой
 * список тех, кто выполнил требуемые критерии (комментарий/лайк).
 */
export function FacebookEngagementCard({ source, primaryColor, secondaryColor }: FacebookEngagementCardProps) {
  const [actions, setActions] = useState<FacebookEngagement[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  const loadActions = async () => {
    setIsLoading(true);
    const data = await fetchFacebookEngagement(
      source.requiredCriteria,
      source.qualifiedCount ? source.qualifiedCount + 15 : 26
    );
    setActions(data);
    setIsLoading(false);
  };
  ...
  const { qualified, qualifiedActions } = summarizeFacebookEngagement(actions);
    ...
    <div className="rounded-3xl glass border border-white/10 shadow-glass overflow-hidden">
      <div className="p-5 flex items-center gap-3 border-b border-white/10">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-white">{source.pageUsername}</p>
