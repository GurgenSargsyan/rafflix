...
import { Send, Heart, Repeat2, MessageCircle, ExternalLink, RefreshCw, Users } from "lucide-react";
import { fetchChannelActions, summarizeActions, criteriaLabel } from "@/lib/services/telegram";
import { formatNumber } from "@/lib/utils";
import type { TelegramAction, TelegramCriteriaType, TelegramSource } from "@/types";

interface TelegramActionsCardProps {
...
/**
 * Публичная витрина розыгрыша "по Telegram-каналу": показывает пост и живой
 * список подписчиков, выполнивших требуемые критерии (лайк/репост/комментарий).
 * Участие не требует формы — достаточно провзаимодействовать с постом в канале.
 */
export function TelegramActionsCard({ source, primaryColor, secondaryColor }: TelegramActionsCardProps) {
  const [actions, setActions] = useState<TelegramAction[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  const loadActions = async () => {
    setIsLoading(true);
    const data = await fetchChannelActions(
      source.requiredCriteria,
      source.qualifiedCount ? source.qualifiedCount + 20 : 35
    );
    setActions(data);
    setIsLoading(false);
  };
  ...
  const { qualified, qualifiedActions } = summarizeActions(actions);
  ...
  return (
    <div className="rounded-3xl glass border border-white/10 shadow-glass overflow-hidden">
      ...
      <div className="p-5 space-y-4">
        ...
        >
          <p className="text-sm text-white">Чтобы участвовать, сделай в канале:</p>
            ...
            {source.requiredCriteria.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  {criteriaLabel(c)}
                  {c === "reaction" && source.requiredReactionEmoji && ` ${source.requiredReactionEmoji}`}
                </span>
              );
            })}
          </div>
          ...
          {qualifiedActions.slice(0, 12).map((a, i) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {a.commentText && <p className="text-xs text-white/45 truncate">{a.commentText}</p>}
              </div>
